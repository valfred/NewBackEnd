...
const createDynamicPdf = async ({ userID, data, piste }) => {
  try {
    ...
    //fin du doc -> page IP + page logo
    const endPdfPath = await endDocumentPdf({ userID, piste, data })
...
const createPistePdf = async ({ userID, piste, data }) => {
  ...
  //sélection des pistes par l'utilisateur
  if (condition) {
    await merger.add(templatePath, [16, 27]) // les pistes de réflexion  
  } else {
...
//fin doc page IP + page logo

const endDocumentPdf = async ({ userID, piste, data }) => {
  const outputDir = path.join(__dirname, `../static/pdf/generated/${userID}`)
  const outputPath = `${outputDir}/piste.pdf`
  const merger = new PDFMerger()
  await merger.add(templatePath, [28, 29]) // les pistes de réflexion  
  await merger.save(outputPath)
  return outputPath
}
